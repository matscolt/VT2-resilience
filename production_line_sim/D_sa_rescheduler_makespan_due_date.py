...
def _baseline_order(units: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return sorted(
        units,
        key=lambda unit: (
            _safe_float(unit.get("due_date_s"), float("inf")),
...
def _evaluate_sequence(
    ...
    due_date_inversion_count = 0
    total_tardiness_s = 0.0
    completed_due_dates: list[float] = []
    ...
    for unit in sequence:
        ...
        due_date_s = _safe_float(unit.get("due_date_s"), float("inf"))
        ...
        if math.isfinite(due_date_s):
            total_tardiness_s += max(0.0, completion_time_s - due_date_s)
            due_date_inversion_count += sum(1 for previous_due_date_s in completed_due_dates if previous_due_date_s > due_date_s)
            completed_due_dates.append(due_date_s)

    due_date_tardiness_weight = _safe_float(config.get("due_date_tardiness_weight", 0.25), 0.25)
    due_date_sequence_penalty_s = _safe_float(config.get("due_date_sequence_penalty_s", 30.0), 30.0)
    return makespan_s + due_date_tardiness_weight * total_tardiness_s + due_date_sequence_penalty_s * float(due_date_inversion_count)
