...
def is_schedule_feasible(
    ...
    """
    A schedule is feasible if all scheduled units are completed
    before the selected rolling horizon ends.
    """
    ...
    return makespan <= horizon_info["horizon_end_s"]
...
def run_ga(
    ...
    for generation in range(GENERATIONS):
        ...
        for chromosome_index, order_chromosome in enumerate(
            ...
            is_feasible = is_schedule_feasible(
            ...
            feasible_marker = "FEASIBLE" if is_feasible else "NOT FEASIBLE"
            ...
            print(
                ...
                f"{feasible_marker} | "
                f"Makespan {simulation_result.get('makespan', 'N/A')} | "
                f"HorizonEnd {horizon_info['horizon_end_s']} | "
...
def run_ga_with_adaptive_horizon(
    ...
    """
    Try rolling horizons in this order:
    1 day -> 3 days -> 5 days.

    Feasible means all scheduled units are completed before horizon_end_s.
    If no feasible plan is found, the best 5-day plan is used anyway.
    """

    last_result = None
    last_horizon_info = None
    last_orders = None
    last_units = None
    last_order_units = None
    ...
    for lookahead_days in [1, 3, 5]:
        ...
        last_result = result
        last_horizon_info = horizon_info
        last_orders = orders
        last_units = units
        last_order_units = order_units
    ...
    print(
        "\nWARNING: No feasible schedule found with 1, 3, or 5 day horizon. "
        "Using best available 5-day plan anyway."
    )
...
def main(
    ...
    print(f"Schedule feasible within selected horizon: {best_is_feasible}")
