...
def resolve_line_layout_path(
    ...
    if selected_layout_name is None or str(selected_layout_name).strip() == "":
        for candidate in [
            (batch_dir / LINE_LAYOUT_FILENAME) if batch_dir is not None else None,
            data_dir / LINE_LAYOUT_FILENAME,
            layouts_dir / LINE_LAYOUT_FILENAME,
        ]:
    ...
    if batch_dir is not None:
        search_locations.append(batch_dir)
    search_locations.extend([layouts_dir, data_dir])
...
def resolve_disruption_path(batch_dir: Path | None) -> Path | None:
    if batch_dir is None:
        return None
    candidate = batch_dir / DISRUPTION_FILENAME
...
def resolve_timed_disruption_csv_path(batch_dir: Path | None) -> Path | None:
    ...
    if exact_candidate.exists() and exact_candidate.is_file() and exact_candidate.suffix.lower() == ".csv":
    ...
    csv_candidates.extend(
        sorted(
            path
            for path in batch_dir.glob("disruption_list*.csv")
    ...
    csv_candidates.extend(
        sorted(
            path
            for path in batch_dir.glob("disruptions*.csv")
            if path.is_file() and path.suffix.lower() == ".csv"
        )
    )
...
def run_simulation(
        ...
        for emergency_order in timed_disruption_data.get("emergency_orders", []):
            ...
            for variant_value, quantity_value in emergency_order.get("variants", []):
                for _ in range(int(quantity_value)):
                    ...
                    emergency_due_date_value = emergency_order.get("due_date_s", float("inf"))
                    unit_due_dates.append(float("inf") if emergency_due_date_value is None else float(emergency_due_date_value))
            ...
            unreleased_units.append(
                {
                    ...
                    "due_date_s": float("inf") if unit_due_dates[unit_index] is None else float(unit_due_dates[unit_index]),
    ...
    def try_allocate_replacement_unit(
        ...
        unit_due_dates.append(float(unit_due_dates[failed_unit_index]) if unit_due_dates[failed_unit_index] is not None else float("inf"))
...
def main() -> None:
    ...
    disruption_path = resolve_disruption_path(batch_dir=batch_dir_for_layout)
    timed_disruption_csv_path = resolve_timed_disruption_csv_path(batch_dir=batch_dir_for_layout)
