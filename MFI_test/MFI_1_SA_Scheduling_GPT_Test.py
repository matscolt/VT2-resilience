...
# SA keeps the same order-level chromosome representation as the GA,
# but uses one solution at a time instead of a population.
SA_ITERATIONS = 120
SA_INITIAL_TEMPERATURE = 1.0
SA_COOLING_RATE = 0.97
SA_MIN_TEMPERATURE = 1e-4
SA_INSERT_PROBABILITY = 0.7

# Legacy GA constants are kept only because the original helper functions
# are left unchanged in the file. They are not used by run_sa().
SWAPS = 3
POPULATION_SIZE = 6
GENERATIONS = 4
ELITE_SIZE = 2
...
def create_sa_initial_solution(orders: List[Order]):
    """Create the initial SA solution using the same EDD seed logic as the GA."""

    return create_order_seed(orders)


def swap_neighbor(chrom):
    """Create a neighbour by swapping two orders."""
...
def insert_neighbor(chrom):
    """Create a neighbour by moving one order to another position."""
...
def create_sa_neighbor(chrom):
    """Create a new neighbouring schedule from the current order sequence."""

    if random.random() < SA_INSERT_PROBABILITY:
        return insert_neighbor(chrom)

    return swap_neighbor(chrom)
...
def run_sa(
    ...
    current_solution = create_sa_initial_solution(
        orders
    )
    ...
    current_unit_sequence = order_chromosome_to_unit_sequence(
        current_solution,
        order_units
    )

    current_simulation_result, _ = evaluate_schedule_with_simulator(
    ...
    print("================================================")
    print(
        f"Initial fitness: {current_fitness:.4f} | "
        f"Initial temperature: {temperature:.4f}"
    )

    for iteration in range(1, SA_ITERATIONS + 1):

        candidate_solution = create_sa_neighbor(
            current_solution
        )

        candidate_unit_sequence = order_chromosome_to_unit_sequence(
            candidate_solution,
            order_units
        )

        candidate_simulation_result, _ = evaluate_schedule_with_simulator(
        ...
        if delta < 0:
            ...
            accept_probability = math.exp(
                -delta / max(temperature, SA_MIN_TEMPERATURE)
            )
        ...
        print(
            f"Iter {iteration:03d}/{SA_ITERATIONS} | "
            f"Temp {temperature:10.4f} | "
            f"Current {current_fitness:12.4f} | "
            f"Candidate {candidate_fitness:12.4f} | "
            f"Best {best_fitness:12.4f} | "
            f"Late {candidate_fitness_result['late_orders']:2d} | "
            f"MaxDay {candidate_fitness_result['max_tardiness_days']:6.2f} | "
            f"Accepted {str(accept):5s} | "
            f"{best_marker}"
        )
    ...
    print(f"Accepted moves: {accepted_moves}/{SA_ITERATIONS}")
    print(f"Improving moves: {improving_moves}/{SA_ITERATIONS}")
    print(f"Best order sequence: {best_order_solution}")
    print("================================================\n")
